...
(function($) {
  var AjaxAction = function(el, options) {
    this.$element = $(el);
    this.options = $.extend({}, $.fn.ajaxAction.defaults, options);
  ...
  AjaxAction.prototype = {
    ...
    perform: function(data, callback, continueAfterCallback) {
      ...
      $.ajax({
        url: $.fn.ajaxAction.defaults[this.$element.data('action')+'Url'],
        type: 'POST',
        'data': data,
        success: function(data) {
  ...
  $.fn.ajaxAction = function(option) {
    return this.each(function() {   
      var $this = $(this),
          data = $this.data('ajaxaction'),
          options = typeof option == 'object' && option;
      if (!data) $this.data('ajaxaction', (data = new AjaxAction(this))); // create new instance for this dom element if doesn't exist
      if (typeof option == 'string') data[option](); // call initial function based on option parameter
    })
  }
  
  $.fn.ajaxAction.defaults = {
    loadingText: 'loading...',
  ...
  $.fn.ajaxAction.Constructor = AjaxAction
  
  $(function () {
    $('body').on('click.ajaxAction', '[data-action]', function(e) {
      e.preventDefault();
      if (!$(this).hasClass('disabled'))
        $(this).ajaxAction($(this).data('action'));
    });
  })  
})(window.jQuery);